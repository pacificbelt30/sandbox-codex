...
grant_type=refresh_token&refresh_token=<空>&client_id=app_EMoamEEZ73f0CkXaXp7hrann
```
...
proxy, _ := authproxy.NewProxy(authproxy.Config{
    TokenTTL:   3600,
    Verbose:    true,
    ListenAddr: "192.168.200.1:0", // dock-net gateway (F-NET-04 対応)
})
...
// エンドポイント確認
fmt.Println(proxy.Endpoint()) // "http://192.168.200.1:XXXXX"
