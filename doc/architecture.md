...
ホスト (Auth Proxy)                  コンテナ
  API キーをメモリ保持
  ↓
  CODEX_TOKEN=cdx-xxx        ──────▶ GET /token → {"api_key": "sk-xxx"}
  OPENAI_BASE_URL=proxy/v1           export OPENAI_API_KEY=sk-xxx
                                     exec codex
                                     ↓
...
ホスト (Auth Proxy)                  コンテナ
  ...
  refresh_token (ホスト内のみ) ─────▶ GET /token
                                       ...
                                       write ~/.codex/auth.json (refresh_token は空)
  ...
  新 access_token を返す (refresh_token は除外)
```

> **重要**: OAuth モードでは `refresh_token` はコンテナに渡りません。
> コンテナが侵害されても攻撃者はトークンを更新できず、access_token の TTL が切れれば無効になります。
