...
│  (保護)       (Docker container:18080)     (cdx-xxxx)        TTL付き      │
│               ↑ codex-auth-proxy:18080 で到達可能        │
│                   │                                         │
│                   │ API リクエスト中継時に                   │
...
> F-NET-04 (Auth Proxy がコンテナから到達不能) は Auth Proxy を `dock-net` 上の専用コンテナ (`codex-auth-proxy:18080`) として常駐させる構成で解決。ワーカーは固定 URL `http://codex-auth-proxy:18080` 経由で到達する。
