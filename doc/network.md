...
| サブネット | `192.168.200.0/24` | コンテナに割り当てるアドレス空間 |
...
### F-NET-04: Auth Proxy への到達不可

**問題**: Auth Proxy は `127.0.0.1`（ホストのループバック）でリッスンしていますが、
`dock-net`（192.168.200.0/24）上のコンテナはホストの `127.0.0.1` に到達できません。

```
コンテナ ──✗──▶ 127.0.0.1:PORT (Auth Proxy)   ← 到達不可！
```

**影響**: コンテナ内の `entrypoint.sh` が Auth Proxy から認証情報を取得できません。

**SRS の設計意図**: Auth Proxy は dock-net 上の特定アドレス（例: `192.168.200.1`）でリッスンし、
コンテナのみがアクセスできるように設計する。

**回避策 (未実装)**:
- Auth Proxy を `0.0.0.0` または `192.168.200.1` でリッスン
- Docker の host networking を使用（セキュリティ低下）
- ホストへのルート追加
