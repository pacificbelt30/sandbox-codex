...
| ChatGPT OAuth（ブラウザ / デバイスコードフロー） | ✅ 対応 | `~/.codex/auth.json` をコンテナに bind mount |

#### 実装方式: auth.json の bind mount

OAuth 認証時（`~/.codex/auth.json` に `refresh_token` フィールドが存在する場合）は、Auth Proxy による API キー注入を行わず、代わりに auth.json をコンテナ内の `/home/codex/.codex/auth.json` に bind mount する。コンテナ内の Codex CLI がホストと同じ auth.json を直接読み書きするため、トークンの自動リフレッシュも通常どおり機能する。

```
ホスト                               コンテナ
~/.codex/auth.json  ──bind mount──▶  /home/codex/.codex/auth.json
  { access_token,                       Codex CLI が直接読み書き
    refresh_token, ... }                トークンリフレッシュも機能
```

#### セキュリティ上のトレードオフ

この実装は **F-AUTH-01**（本物の認証情報をコンテナに渡さない）を意図的に違反する。auth.json にはアクセストークンとリフレッシュトークンが平文で含まれるため、コンテナ内のプロセスがこれらを読み取ることが可能になる。API キーモードの Auth Proxy による保護（短命トークンへの変換）は OAuth モードでは提供されない。
