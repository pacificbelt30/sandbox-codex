...
| ID | 要件 | 状態 | 備考 |
|----|------|------|------|
| F-AUTH-01 | auth.json をコンテナに配置しない | ✅ | |
...
#### 現状: ChatGPT サブスクリプションは**非対応**

codex-dock の Auth Proxy は API キー（`OPENAI_API_KEY`）のみを想定した設計であり、ChatGPT OAuth セッションとは**根本的に非互換**である。

| Codex CLI 認証方式 | codex-dock 対応状況 | 理由 |
|-------------------|-------------------|------|
| API キー (`OPENAI_API_KEY`) | ✅ 対応 | Auth Proxy が API キーをトークンに変換してコンテナに注入 |
| ChatGPT OAuth（ブラウザフロー） | ❌ 非対応 | コンテナ内でブラウザが起動できない。OAuth アクセストークンは `OPENAI_API_KEY` として注入不可 |
| ChatGPT デバイスコードフロー（Beta） | ❌ 非対応 | ヘッドレス環境向けの代替フローだが、codex-dock がこのフローを仲介する仕組みがない |

#### 技術的な背景

1. **トークン種別の違い**: OAuth セッションが発行するアクセストークンは Bearer Token（JWT）であり、API キーとはフォーマットが異なる。`OPENAI_API_KEY` 環境変数に OAuth トークンをセットしても Codex は API キー認証として解釈できない。

2. **トークンリフレッシュ**: OAuth セッションは有効期限が短く（5分以内に自動リフレッシュ）、リフレッシュトークンを使った更新ロジックが必要。Auth Proxy は現状このリフレッシュ機構を持たない。

3. **auth.json の内容**: ChatGPT OAuth でログインした場合の `~/.codex/auth.json` は `access_token` / `refresh_token` 形式であり、現在の `readCodexAuthJSON()` が検索するキー（`OPENAI_API_KEY`, `api_key`, `key`, `token`）と一致しない可能性が高い。

4. **セキュリティ上の制約**: auth.json ごとコンテナにマウントすれば一時的に動作するが、F-AUTH-01/02 の要件（auth.json・本物の認証情報をコンテナに渡さない）に違反する。

#### 対応に必要な実装

ChatGPT サブスクリプション対応を行うには以下の実装が必要：

- Auth Proxy に OAuth アクセストークンの保持・リフレッシュ機能を追加する
- コンテナの `entrypoint.sh` が `OPENAI_API_KEY` ではなく、Codex が期待する OAuth セッション形式でトークンを受け取れるよう変更する（または Auth Proxy が OpenAI API の互換エンドポイントとして動作する）
- あるいは、`codex login --device-auth` を codex-dock が仲介し、コンテナ起動前にホスト側でデバイスコードフローを完了させてセッショントークンを取得する方式を実装する
