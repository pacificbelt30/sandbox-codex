...
| F-NET-02 | コンテナ→ホスト通信の遮断 | ⚠️ | Linux では `DOCKER-USER` + `iptables` で private/link-local 宛を遮断。`run` は適用失敗時に Warning で継続し、`firewall create` で明示適用可能。macOS / Windows では同等の自動制御は未実装。 |
